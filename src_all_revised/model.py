...
from utils.tools_model import VisualEnhancementModule, AdaptiveFusion, AVPrompt
...
class CLIPVAD(nn.Module):
    def __init__(self,
        ...
        self.device = device
        self.temperature = 0.07  # 온도 파라미터 추가
        ...
        # 시각 강화 모듈 추가
        self.visual_enhancement = VisualEnhancementModule(visual_width)
        
        # 적응적 융합 모듈 추가
        self.adaptive_fusion = AdaptiveFusion(dim=visual_width)
        
        # AV 프롬프트 모듈 추가
        self.av_prompt = AVPrompt(dim=visual_width, num_class=num_class)
        ...
        # 수정: 입력 차원을 visual_width로 변경 (적응적 융합 모듈 사용)
    ...
    def encode_video(self, images, audio, padding_mask, lengths):
        ...
        # 시각 강화 모듈 적용
        x_visual = self.visual_enhancement(x_visual)
    ...
    def forward(self, visual, audio, padding_mask, text, lengths, return_fused=False):
        # 시각 및 오디오 특징 인코딩
        ...
        # 적응적 융합 적용
        fused_features = self.adaptive_fusion(visual_features, audio_features)  # [B, 256, 512]
        
        # return_fused=True인 경우 융합된 특징만 반환 (Stage 2 학습용)
        if return_fused:
            return None, None, None, None, None, fused_features
        
        # 융합된 특징에 대한 분류
        logits_av_3d = self.av_classifier(fused_features)  # [B, 256, 1]
        logits_av = logits_av_3d.squeeze(-1)  # [B, 256]
        
        # 원시 로짓 값 사용 (시그모이드 변환 없음)
        conf = logits_av_3d  # [B, 256, 1]
        ...
        text_features = text_features_ori.unsqueeze(0).expand(batch_size, -1, -1)  # [B, N, 512]
        
        # AVPrompt 모듈 적용: 동적 프롬프트 주입
        enhanced_text_features = self.av_prompt(
            x_av=fused_features,      # 융합된 시퀀스 특징 [B, T, D]
            conf=conf,                # 원시 로짓 값 [B, T, 1]
            x_c=text_features         # 클래스별 원본 텍스트 임베딩 [B, C, D]
        )  # [B, N, 512]
        
        # 특징 정규화
        visual_features_norm = visual_features / visual_features.norm(dim=-1, keepdim=True)
        enhanced_text_norm = enhanced_text_features / enhanced_text_features.norm(dim=-1, keepdim=True)
        
        # 강화된 텍스트 특징으로 유사도 계산
        enhanced_text_norm = enhanced_text_norm.permute(0, 2, 1)  # [B, 512, N]
        logits2 = visual_features_norm @ enhanced_text_norm / self.temperature  # [B, 256, N]
        
        # 수정: 5개 값만 반환
        return text_features_ori, logits_av, logits2, logits_visual, logits_audio